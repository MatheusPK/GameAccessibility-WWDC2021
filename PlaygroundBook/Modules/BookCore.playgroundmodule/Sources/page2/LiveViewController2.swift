...
@objc(BookCore_LiveViewController2)
public class LiveViewController2: UIViewController, PlaygroundLiveViewMessageHandler, PlaygroundLiveViewSafeAreaContainer {
    
    var gameScene: GameScene2!
    var faceTrackingView: FaceTrackingView!
    ...
    public override func viewDidLoad() {
        ...
        super.viewDidLoad()
            gameScene = GameScene2(size: self.view.frame.size)
    ...
    public func receive(_ message: PlaygroundValue) {
        ...
        switch message {
            case let .array(info):
                var newArray:[Int] = []
                for elem in info {
                    guard case .integer(let newInt) = elem else { return }
                    newArray.append(Int(newInt))
                }
                print(info)
                gameScene.faceConfig = newArray
                gameScene.resetScene()
                //gameScene.addChild(SKSpriteNode(texture: nil, color: .black, size: CGSize(width: 100, height: 200)))
            default:
